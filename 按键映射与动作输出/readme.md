...
`keybinds.lua` 遍历 `Fuyutsui.actionBars`（定义于 `core/config.lua`），利用每个动作条的 `startSlot`、`endSlot`、`bindingPrefix` 三个属性来匹配槽位和按键绑定前缀（如 `ACTIONBUTTON`、`MULTIACTIONBAR1BUTTON` 等）。
