...
4. **注册职业逻辑模块**：在 `logic_gui.py` 中通过 `_load_logic_module()` 导入新逻辑模块（逻辑文件需创建在 `class/` 目录下），并在 `LOGIC_FUNCS_BY_CLASS` 字典中注册 `class_id` 到逻辑函数的映射。若不注册，主循环通过 `LOGIC_FUNCS_BY_CLASS.get(class_id, _default_logic)` 查找时会回退到 `_default_logic`，导致自定义逻辑静默失效。
5. **在游戏中绑定宏**：为 keymap 中每个热键创建对应的游戏内宏（包含目标选择和施法指令），并将宏绑定到 keymap 中声明的按键上。
