...
keymap 文件统一放在 `Fuyutsui/Fuyutsui/keymap/` 目录下。默认存在一个 `keymap.yml`（通常为空），同时每个职业有各自的 keymap 文件（如 `priest.yml`、`warrior.yml` 等）。
...
2. 如果 `class_id` 为 `None`，回退到默认的 `keymap.yml`。
...
`get_hotkey()` 同时支持中英文字段名：`spell` 与 `技能` 等效，`hotkey` 与 `热键` 等效。优先使用中文键名。
...
| 辅助函数 | 返回 |
|----------|------|
| `get_lowest_health_unit(state_dict, health_threshold)` | 生命值最低的单位编号 |
| `get_unit_with_dispel_type(state_dict, dispel_type)` | 有指定驱散类型的单位编号 |
| `get_unit_with_role(state_dict, role)` | 指定职责（坦克/治疗/输出）的单位编号 |
| `get_lowest_health_unit_without_aura(state_dict, aura_name, health_threshold)` | 没有指定光环且生命值最低的单位编号 |
| `get_unit_with_aura(state_dict, aura_name)` | 拥有指定光环的单位编号 |
| …以及其他 `get_*` 函数 | 对应各种筛选条件 |

这些函数返回的单位编号（`"1"`~`"30"` 字符串）被转为整数后传给 `get_hotkey()`。
...
职业逻辑中通过 `int(state_dict.get("延迟", 0))` 读取。当 `延迟 > 0` 时，逻辑函数通常会跳过所有按键决策（返回 `None`），实现"等待期间不做任何操作"的效果。
...
select_keymap_for_class(class_id)   ─── 切换 keymap 路径，清空缓存
    ...
    ▼
职业逻辑 *_logic.py:
    unit = get_lowest_health_unit(state_dict)   ─── 获取目标单位编号
    action_hotkey = get_hotkey(unit, "技能名")  ─── 查找热键字符串
