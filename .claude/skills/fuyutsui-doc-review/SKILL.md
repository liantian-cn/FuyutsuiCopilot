...
### 第二步：分类
- **三方共识**（全员发现）：3 位审查者全部独立指出了同一问题 → 最高置信度，无论严重程度高低都必须纳入修改
- **双方共识**（多数发现）：2 位审查者独立指出了同一问题 → 高置信度
- **差异发现**（分歧）：只有 1 位审查者提出了该问题 → 需要复查验证

分类时，在 `found_by` 数组中列出发现该问题的审查者代号。三方共识的 `found_by` 必须包含全部三个代号 ["Alpha", "Beta", "Gamma"]。
  ...
  "common": [
    {
      ...
      "found_by": ["Alpha", "Beta", "Gamma"]  // 三方共识：3人都发现；双方共识则只有2个代号
    }
  ],
  "differences": [
    {
...
### 一审共识问题（Alpha、Beta、Gamma 中 ≥2 位共同发现）：
> 注意：`found_by` 为 ["Alpha", "Beta", "Gamma"] 的是**三方共识**，只有 2 个的是**双方共识**。
...
### 第零步：三方共识直接纳入修改（硬性规则）
一审共识问题中，`found_by` 包含全部三位审查者（["Alpha", "Beta", "Gamma"]）的项，视为**三方共识**。
**无论其 severity 是"高"、"中"还是"低"，三方共识项必须无条件全部纳入修改列表。**
理由：三双眼睛独立发现同一问题，说明该问题对读者理解有实际影响，严重程度标签在此情况下已不重要。
三方共识项的 `priority` 至少设为 "必须修改"。

### 第一步：验证双方共识问题
快速阅读 \`${BASE}/Fuyutsui/\` 下相关源码，验证一审中只有 2 位审查者发现的共识问题是否确实存在。
如果某个双方共识在源码层面站不住脚，标注为误判并说明原因。双方共识项的修改优先级根据实际影响判断。
...
## 优先级执行
- "必须修改" → 全部修改（含 Theta 已判定的三方共识项）
- "建议修改" → 全部修改
- "可选" → 酌情修改，以不破坏文档流畅性为准
- 重要：Theta 传来的修改意见已经过三方共识规则筛选，请信任其优先级判断
