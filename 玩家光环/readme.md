...
3.5 事件处理函数在 main.lua 中先通过 issecretvalue/isSec 过滤受保护的 spellId：SPELL_UPDATE_COOLDOWN、SPELL_UPDATE_ICON、UNIT_SPELLCAST_SUCCEEDED 和 UNIT_SPELLCAST_FAILED 有守卫；GLOW/OVERLAY 类和 COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED 处理函数无守卫。其中 UNIT_SPELLCAST_FAILED 的守卫服务于 updateSpellFailed（施法失败通知）路径而非光环更新路径，因此不影响光环状态机逻辑，但对理解 spellId 过滤的完整边界有参考意义。被过滤的 spellId 不会触发任何光环更新。
...
注：auras.lua 源码注释中未列出「图标发光显示」，但 config.lua 中定义了此键且无羁邪怒使用了它。该注释已过时——完整键列表以 config.lua 为准。
...
| `duration` | 默认持续时间，单位：秒（像素量化后精度约 ±0.5 秒） |
...
- `0`：初始值，表示还没有被图标覆盖检查更新过（例如圣骑士`神圣军备`的默认值）
- `1`：已经检查过，但当前覆盖法术 ID 不匹配，或者没有可匹配的覆盖法术
...
注意 `updateAuraByIconMap` 中 `hasOverride` 局部变量在 `for` 循环外初始化（auras.lua:1168），在遍历同一 (event, spellId) 映射下的多个光环时仅可被设为 `true` 但从不重置为 `false`。一旦某个光环的 `overrideSpellID` 匹配成功，后续同一映射中的所有光环都会看到 `hasOverride = true` 并获得 `isIcon = 2`。当前没有光环受此影响，因为每个 spellId 只对应一个 isIcon 光环。如果扩展为同一 (event, spellId) 下映射多个 isIcon 光环则需要留意此行为。
...
### `name` 和 `spellId` — 当前基本只是标注字段

有些光环条目带有 `name` 和 `spellId` 字段，例如：
...
但在当前源码里，`name` 和 `spellId` 没有参与 `auras.lua` 的事件索引、状态更新或像素输出。事件索引只使用 `addAuras`、`updateAuras`、`removeAuras` 里的 spellId 键；`updateAuraBlocks()` 只按职业块中的 `auraName` 查 `Fuyutsui.Auras[auraName]`，再读取 `showKey` 字段。

所以它们更像人工标注或历史遗留字段，不是"多条目共享显示名"机制。Python 端是否看到一个字段，仍然完全取决于职业 Lua 文件里的 `type = "aura"` 像素和 `config.yml` 的字段名。例如 DK 的 `脓疮毒镰`、`脓疮毒镰2` 在职业块和 `config.yml` 中仍是两个字段；武僧的 `生生不息1`、`生生不息2` 也是两个字段。
...
- 施放 `443421`（未验证：推测为抚慰之雾的关联冷却）时，持续 4 秒（使用事件条目中的 `duration`，其值与光环默认值相同均为 4）。
...
注意'塞纳留斯的梦境'与其他发光光环不同，它同时使用 addAuras 的 e['法术冷却'] step=1（获得层数）、updateAuras 的 e['施法成功'] step=-1（消耗层数）来控制层数（count/step）增删，而图标发光仅作为 removeAuras 的补充清除路径（auras.lua:943-960）。其他发光光环（斩杀高亮等）只有 removeAuras 无 count/step，属于纯时间型。因此塞纳留斯的梦境是混合型控制路径。

注意：无羁邪怒的 addAuras 中的 e['图标发光显示'] 实际不会被 updateAuraByOverlayGlow() 处理，该函数只查询 removeAuras[e['图标发光隐藏']]。该 addAuras 条目是死代码，实际发光激活通过 GLOW_SHOW/HIDE 事件触发 removeAuras 路径。定义发光高亮光环时只在 removeAuras 中配置即可。
...
`COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED` 在技能的覆盖法术 ID 变更时触发。注意此事件的处理函数签名为 `Fuyutsui:COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED(_, baseSpellID, overrideSpellID)`，同时传递基础法术ID和覆盖法术ID两个参数，与其它事件仅传递单一 spellID 不同。`updateAuraBySpellOverride(baseSpellID, overrideSpellID)` 使用这两个参数进行匹配。`updateAuraBySpellOverride()` 检查新的 `overrideSpellID`：

- 如果 `overrideSpellID` 匹配事件条目中配置的 `overrideSpellID`，则设置/刷新 `expirationTime`。
- 如果不匹配，则清除 `expirationTime`。

这主要用于检测"同一个按钮位现在变成了另一个技能"的场景。不过当前 `auras.lua` 中没有看到任何条目使用 `event = e["法术覆盖"]`。萨满`风暴涌流图腾`的 `[5394] = { event = e["图标改变"], overrideSpellID = 1267068 }` 走的是 `SPELL_UPDATE_ICON` / `updateAuraByIcon()` 路径，不是 `COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED` 路径。

注意：COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED 事件在 core.lua:75 仍被注册，main.lua:1535-1537 的处理函数也仍存在并调用 updateAuraBySpellOverride()。只是暂无光环配置消费此路径。
...
如果 `addAuras` 或 `updateAuras` 事件条目里有 `castBar = true`，则 `UNIT_SPELLCAST_SUCCEEDED` 路径要求这次成功施法带读条 ID，避免瞬发或非读条事件误更新层数。守卫逻辑是 `(not info.castBar) or castBarID`：当事件条目没有 castBar 要求（false/nil）时永远通过；当 castBar = true 时要求 castBarID 非 nil 才执行。因此冷却类事件传 nil 也能正常通行。`removeAuras` 当前不读取这个字段，下面会单独说明。
...
`resetCount = false` 表示这一步不会在 `clearAurasFromRemoveMap()` 里直接重置层数。但因为它清除了 `expirationTime`，下一帧 `updateAura()` 进入 `else` 分支时，如果该光环有 `count` 且 `count ~= countMin`，仍会把 `count` 设回 `countMin`。当前源码中 `countMin` 都是 0，因此屏幕提示隐藏后层数通常会在下一帧归零。

### `castBar = true` 防止瞬发误扣层数

当事件条目设置 `castBar = true` 时，只有 `applyAuraMapForSpellEvent()` 和 `updateAuraMapForSpellEvent()` 会要求 `castBarID` 非空才执行：
...
`UNIT_SPELLCAST_SUCCEEDED` 事件的 `castBarID` 参数在瞬发施法时为 `nil`，在读条施法时为非 `nil`。如果 `castBar = true` 放在 `addAuras` 或 `updateAuras` 条目中，含义是："只有这次施法确实是读条施法（不是瞬发）时，才执行这条新增或更新逻辑"。
...
它写在 `removeAuras` 里，而 `removeAuras` 走的是 `clearAurasFromRemoveMap()`；这个函数只看光环名和 `resetCount`，不会读取 `info.step` 或 `info.castBar`。因此这条配置里的 `step = -1` 和 `castBar = true` 在当前实现中都不起作用：治疗波施法成功时会直接清除 `expirationTime`，并因为 `resetCount = true` 把层数重置为 `countMin`。如果想做"只在有读条时扣 1 层"的语义，应放到 `updateAuras` 路径，或者修改 `clearAurasFromRemoveMap()` 的实现。
...
if aura.count and info.step then
    -- 有 count 和 step 时：若 step > 0 刷新持续时间并增加层数；若 step < 0 只减少层数
    if info.step > 0 then
...
注意：applyAuraMapForSpellEvent 中若同时使用 step > 0 和 info.duration，最终 expirationTime 由 aura.duration 决定而非 info.duration。第 1088 行会覆盖第 1082 行的设置。当前无光环使用此组合，但 mod 作者应注意此行为。

区别在于：`addAuras` 总是先刷新持续时间再处理层数（两个逻辑独立）；`updateAuras` 在有 count/step 时：若 step > 0 则刷新持续时间（使用 aura.duration）并增加层数；若 step < 0 只减少层数。无 count/step 时若有 aura.duration 也刷新持续时间。这反映了它们的预期用途：`addAuras` 用于"获得/激活光环"，`updateAuras` 用于"消耗/修改已有光环"。
...
- 对于 `remaining`：原始值是秒级的浮点数（如 7.3 秒），写入时变成 `7.3 / 255 ≈ 0.0286`，但是 `SetColorTexture` 的浮点参数最终在屏幕上还是会落到 0-255 的整数颜色通道。Python 端并没有调用 `math.floor`；它通过 `mss` 读取截图 raw bytes，直接取该像素的 B 通道整数值。因此精度近似为 1 秒/单位，具体取整/舍入由渲染和截图得到的颜色通道决定。
...
| Python `get_info()` | 由调用方决定；`logic_gui.py` 当前约每 0.2 秒调用一次 | 截图并解析像素 |
...
每条 bar 由背景色块、前景白色 `StatusBar` 和一个共享灰色终点组成，结构如下：
...
Lua 端还会创建一个白色 `StatusBar` 前景，`bar:SetValue(val)` 会把从 bar 起点开始的 `val` 个单位覆盖成白色。Python 端读取时，会从段起点或白色块后继续扫描，找到白色填充区域后面的第一个非白色背景像素，取它的 G 通道值，再减 1 得到实际值：
...
例如：当前值为 2 时，前两个单位被白色覆盖，白色之后第一个非白背景像素的 G 通道值通常为 3；Python 再减 1，得到实际 bar 值 2。
...
`CreateAutoLayoutBar()` 内部通过 `spellIdToBar[spellId]` 检查同一 spellId 是否已创建过 bar（block.lua:86-88）。若已存在则直接返回现有 bar，避免重复创建相同技能的 bar：

```lua
if spellIdToBar[spellId] then
    return spellIdToBar[spellId]  -- 已有该技能栏，复用
end
```

新 bar 创建后在 `spellIdToBar[spellId] = bar`（block.lua:153）记录映射。切换专精时 `ClearAllFuyutsuiBars()` 会调用 `wipe(spellIdToBar)`（block.lua:179）清空映射表，确保重新创建所有 bar。

此机制确保：
- 同一 spellId 在插件加载后只对应一条 bar，不会因多次调用 `CreateAutoLayoutBar` 重复创建。
- 切换专精时通过 `wipe` 重建所有 bar，避免旧专精的引用残留。
- Mod 作者在新增 countBar 时无需手动管理去重，但仍需确保为每个技能的唯一 spellId 调用一次创建。
...
|---|---|---|
| 位置 | 第一行 | 第二行（y=-2） |
| 编码 | G=索引, B=值 | 红/红黄背景标记 + 白色前景填充 + 白色后首个非白背景像素的 G 通道值-1 |
| 用途 | 持续时间、层数、isIcon | 技能充能、施法次数 |
| 刷新方式 | `updateAuraBlocks()` 每 0.2 秒 | WoW 事件驱动（`SPELL_UPDATE_USES`、`PLAYER_ENTERING_WORLD`、`SPELL_UPDATE_CHARGES`） |
| 读取方式 | `row_data[step]` | `bar_data[bar]` |

注意 `SPELL_UPDATE_USES` 采用双层注册模式：main.lua:1638-1639 的主事件处理函数 `Fuyutsui:SPELL_UPDATE_USES()` 是空函数（不执行任何逻辑），实际的刷新逻辑在每个 bar 创建时通过 `bar:RegisterEvent('SPELL_UPDATE_USES')` 和 `bar:SetScript('OnEvent', Refresh)` 独立注册（block.lua:145-148）。因此每栏独立响应事件刷新，不依赖主事件处理函数。
...
- `UNIT_AURA` 增量更新时要求 `aura.sourceUnit == "player"`，此时处理 addedAuras 和 updatedAuraInstanceIDs。此外 info.removedAuraInstanceIDs 也会被遍历并以 obj.aura[v] = nil 移除对应的光环实例（main.lua:1785-1789）。这一路径主要用于队伍光环的生命周期管理，不在玩家逻辑光环的主链路上。
...
- Lua 职业文件的像素索引必须和 Python `config.yml` 的 `step` 对齐。例如当前源码中 DK 邪恶专精的 config.yml 里 `脓疮毒镰2` 和 `枯萎凋零` 均使用 `step: 48`（config.yml:498-499），但 DeathKnight.lua 中 `脓疮毒镰2` 在索引 48 而 `枯萎凋零` 在索引 49（DeathKnight.lua:127-128）。这导致 Python 读取 `step: 48` 时实际上读到的是 `脓疮毒镰2` 的数据，而 `枯萎凋零` 的索引 49 永远不被读取。新增字段时需要逐一核对 Lua 类文件的像素索引与 config.yml 的 step 值。
- 如果 Python 字段配置成 `step: bar`，它读的是 `countBars`，不是 `updateAuraBlocks()` 写出的普通光环像素。
- `issecretvalue` 或 `isSec` 在 main.lua 的事件处理层级过滤受保护的 spellId：SPELL_UPDATE_COOLDOWN、SPELL_UPDATE_ICON、UNIT_SPELLCAST_SUCCEEDED 和 UNIT_SPELLCAST_FAILED 有守卫，被过滤的 spellId 不会触发任何光环更新；GLOW/OVERLAY 类和 COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED 的处理函数没有守卫，这些路径不会过滤 spellId。其中 UNIT_SPELLCAST_FAILED 的守卫服务于 updateSpellFailed（施法失败通知）路径而非光环更新路径，因此不影响光环状态机逻辑，但对理解 spellId 过滤的完整边界有参考意义。
...
| 2026-05-30 | 总链路第3.5步（第17行） | Theta 审核：遗漏 UNIT_SPELLCAST_FAILED 的 isSec 守卫 | 将守卫列表补全为 SPELL_UPDATE_COOLDOWN、SPELL_UPDATE_ICON、UNIT_SPELLCAST_SUCCEEDED 和 UNIT_SPELLCAST_FAILED，并注明该事件服务于 updateSpellFailed 路径而非光环更新路径 |
| 2026-05-30 | 发光高亮的轮询确认（第263行附近） | Theta 审核：遗漏塞纳留斯的梦境混合型控制路径说明 | 增加说明塞纳留斯的梦境同时使用 addAuras/updateAuras 控制层数，图标发光仅作为 removeAuras 补充路径，与其他纯时间型发光光环形成对比 |
| 2026-05-30 | 法术覆盖事件（第284行附近） | Theta 审核：遗漏 COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED 双参数签名说明 | 补充该事件处理函数签名传递 baseSpellID 和 overrideSpellID 两个参数，与其它单参数事件不同 |
| 2026-05-30 | isIcon 字段（第144行附近） | Theta 审核：遗漏 hasOverride 变量共享行为说明 | 增加 updateAuraByIconMap 中 hasOverride 局部变量在 for 循环外初始化、同一映射下多个光环共享状态的说明 |
| 2026-05-30 | countBars 编码方式/刷新表格（第581行） | Theta 审核：遗漏 SPELL_UPDATE_USES 双层注册模式说明 | 补充 SPELL_UPDATE_USES 主事件处理函数为空函数、每栏独立注册 OnEvent 刷新的说明 |
| 2026-05-30 | 真实 Aura API（第614行附近） | Theta 审核：遗漏 removedAuraInstanceIDs 处理说明 | 补充 UNIT_AURA 增量更新中 removedAuraInstanceIDs 被遍历移除的说明 |
| 2026-05-30 | 写 mod 时要注意（第625行附近） | Theta 审核：遗漏像素索引错位具体示例 | 增加 DK 邪恶专精下脓疮毒镰2与枯萎凋零 step/索引错位的具体示例 |
| 2026-05-30 | 写 mod 时要注意（第627行附近） | Theta 审核：同一遗漏 | 将守卫列表补全为 SPELL_UPDATE_COOLDOWN、SPELL_UPDATE_ICON、UNIT_SPELLCAST_SUCCEEDED 和 UNIT_SPELLCAST_FAILED，并标注 UNIT_SPELLCAST_FAILED 服务于 updateSpellFailed 路径 |
| 2026-05-30 | countBars 编码方式（bar 创建流程） | Theta 审核：遗漏 spellIdToBar 去重映射表说明 | 新增 spellIdToBar 映射表子节，说明同一 spellId 去重创建、切换专精时 wipe 重建的机制 |
