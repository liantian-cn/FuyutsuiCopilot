...
使用 `isIcon` 的光环在职业块 `showKey` 中可以写 `"isIcon"` 来把状态值输出给 Python。和 `remaining`、`count` 一样，`isIcon` 的值也经过 `v / 255` 写入像素的 B 通道。Python 端读到的 B 通道整数值分别是 0、1、2（因为 `0/255 ≈ 0`、`1/255 ≈ 0.0039`、`2/255 ≈ 0.0078`，量化后恰好对应 0、1、2）。Python 端可以据此区分"尚未初始化"（0）、"当前未激活或不匹配"（1）以及"激活中"（2）。
...
如果 `addAuras` 或 `updateAuras` 事件条目里有 `castBar = true`，则 `UNIT_SPELLCAST_SUCCEEDED` 路径要求这次成功施法带读条 ID，避免瞬发或非读条事件误更新层数。`removeAuras` 当前不读取这个字段，下面会单独说明。
